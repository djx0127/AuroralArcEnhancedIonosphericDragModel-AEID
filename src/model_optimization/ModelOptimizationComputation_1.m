function [y_j,y_star] = ModelOptimizationComputation_1(c_current,elem,track)
...
elem_dadt_fill = filloutliers(elem_dadt,'linear','movmedian',20); % Remove outliers (MAD)
elem_dadt_fill = smoothdata(elem_dadt_fill,'rlowess',20);        % Denoise (RLOESS)

% Theoretical decay rate (from orbit-fit Delta a)
track_dadt = abs(track_delt_a) ./ Delt_t;
track_dadt_fill = filloutliers(track_dadt,'linear','movmedian',30); % Remove outliers (MAD)
track_dadt_fill = smoothdata(track_dadt_fill,'rlowess',30);        % Denoise (RLOESS)
...
function a = Amplitude_F107NDst_HighSolar(c)
% Amplitude modulation function for YG-9 in high solar activity
a = c;
end

function DVeq = Delta_Veq_calc(MLAT,MLT,c)
% Compute extreme charging voltage increment from parameters
H_MLT  = H_mlt_gaussian(MLT);
G_MLAT = G_MLAT_sigmoid(MLAT);
A_SunMag = Amplitude_F107NDst_HighSolar(c);
