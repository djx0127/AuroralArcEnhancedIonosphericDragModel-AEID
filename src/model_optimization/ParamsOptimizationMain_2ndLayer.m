...
%  ---------------------------------------------------------------
%  Case: YG-16/17/20/25 – Second-Level Optimization
%  ===============================================================
clear;
close all;
...
m21 = 780; % Mass of YG-31 A-satellite
m22 = 720; % Mass of YG-31 B/C-satellites
...
TLE_data_files = {
    ...
    'data/TLE_IRI_dataset/YG20_IRIData1.mat', 'YG-20';  % Contains NaN values
    'data/TLE_IRI_dataset/YG25_IRIData1.mat', 'YG-25';
};

% Use a cell array to store processed data of all satellites
...
for i = 1:length(TLE_data_files)
    ...
    % Use A-satellite as representative in global optimization
    raw_data = Data_A;
    [elem,track] = IRIDataLoading(raw_data,allData);
  
    % Initial parameter for this model (no optimization yet)
    c_0 = [0,1,1,1];
    [y_init,y_star] = ModelOptimizationComputation_2(c_0,elem,track);
  
    % Store processed data for this satellite
    ...
    [all_TLE_data{i,1}.y_init_A,all_TLE_data{i,1}.y_star_A] ...
        = ModelOptimizationComputation_2(c_0,elem_A,track_A);
    [y_init_B,y_star_B] ...
        = ModelOptimizationComputation_2(c_0,elem_B,track_B);
    [y_init_C,y_star_C] ...
        = ModelOptimizationComputation_2(c_0,elem_C,track_C);
...
% ---------------------------------------------------------------
% Parameter bounds
lb = [  1, 0.01,  50, 0.001];   % Lower bounds
rb = [400,    1, 200,   0.5];   % Upper bounds

opts = optimoptions('ga', 'Display', 'iter', ...       
    'MaxGenerations', 100, 'PopulationSize', 200, 'UseParallel', true);         

num_vars = 4; % Number of parameters to be optimized
...
for i = 1:length(TLE_data_files)
    ...
    [all_TLE_data{i,1}.y_model_A,~] = ...
        ModelOptimizationComputation_2(optimal_params,elem_A,track_A);
  
    % DTW-based metric after optimization
    ...
    % Fill area for [lower_bound, upper_bound]
    ...
    subplot(2,1,2)
    yyaxis left
    h_resm = plot(track_A.time_jday-time_start, res_model, ...
                  'b-','LineWidth',1.5);
    hold on; grid on; box on;
    h_reso = plot(track_A.time_jday-time_start, res_orign, ...
                  'k-','LineWidth',1.5);
    xlim([0,time_stop-time_start]); ylim([-1,2.5]);  
    ...
    hold on; grid on; box on;
    xlim([0,time_stop-time_start]); ylim([0,5e4]);
    ylabel('Ni [cm^{-3}]');
...
total_error = 0;
    for i = 1:length(Data_All)
        ...
        [y_model,~] = ModelOptimizationComputation_2(c_params,elem,track);
